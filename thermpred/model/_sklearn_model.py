...
class SklearnModel(_base_model.BaseModel, abc.ABC):
    """
    Parent class based on :obj:`~thermpred.model._base_model.BaseModel` for all models with a sklearn-like API to share
    functionalities. See :obj:`~thermpred.model._base_model.BaseModel` for more information.
    ...
    **Attributes**
    ...
    def predict(self, X_in: np.array) -> (np.array, np.array):
        ...
        See :obj:`~thermpred.model._base_model.BaseModel` for more information.
        """
        return np.reshape(self.model.predict(X_in), (-1, 1)), np.reshape(self.model.predict_proba(X_in)[:, 1], (-1, 1))

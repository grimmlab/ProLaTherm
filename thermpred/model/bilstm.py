...
class BidirectionalLstm(_torch_model.TorchModel):
    """
    Implementation of a class for a bidirectional lstm
    ...
    def define_model(self) -> torch.nn.Sequential:
        """
        Definition of an Bi LSTM network.
        """
        model = []
        act_function = self.get_torch_object_for_string(string_to_get=self.suggest_hyperparam_to_optuna('act_function'))
        embedding_dim = 2**self.suggest_hyperparam_to_optuna('embedding_dim_exp_lstm')
        model.append(
        ...
        model.append(
            _torch_model.LstmWithFlattenParams(
                ...
                dropout=p, bidirectional=True)
        )
        model.append(_torch_model.ExtractTensor(bidirectional=True))
        model.append(torch.nn.Dropout(p=p))
        out_features = 2 * int(max(1, hidden_size * self.suggest_hyperparam_to_optuna('initial_units_factor')))
        model.append(torch.nn.Linear(in_features=2*hidden_size, out_features=out_features))

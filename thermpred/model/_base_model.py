...
class BaseModel(abc.ABC):
    ...
    Every model must be based on :obj:`~thermpred.model._base_model.BaseModel` directly or BaseModel's child classes,
    e.g. :obj:`~thermpred.model._sklearn_model.SklearnModel` or :obj:`~thermpred.model._torch_model.TorchModel`
    ...
    **Attributes**
        ...
        *Class attributes*
        ...
        - featureset (*str*): the featureset to use for this model ('features', 'sequence' or 'structure')
        ...
        *Instance attributes*
        ...
        - all_hyperparams (*dict*): dictionary with all hyperparameters with related info that can be tuned (structure see :obj:`~thermpred.model._base_model.BaseModel.define_hyperparams_to_tune`)
        - model: model object
    ...
    @abc.abstractmethod
    def featureset(cls):
        """the featureset to use for this model"""
    ...
    def predict(self, X_in: np.array) -> (np.array, np.array):
        ...
        :return: numpy array with the predicted values and numpy array with predict scores
        """

    @abc.abstractmethod
    def train_val_loop(self, X_train: np.array, y_train: np.array, X_val: np.array, y_val: np.array) -> (np.array, np.array):
        """
        ...
        :return: predictions on validation set, labels and predict scores
        """
    ...
    def suggest_hyperparam_to_optuna(self, hyperparam_name: str):
        ...
        :param hyperparam_name: name of the hyperparameter to be tuned (see :obj:`~thermpred.model._base_model.BaseModel.define_hyperparams_to_tune`)
    ...
    def save_model(self, path: pathlib.Path, filename: str):
        ...
        (can be loaded with :obj:`~thermpred.model._model_functions.load_model`)

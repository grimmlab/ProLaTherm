...
from . import _base_model, _torch_model
...
def load_retrain_model(path: pathlib.Path, filename: str, X_retrain: np.array, y_retrain: np.array,
                       early_stopping_point: int = None, lr_reduce_points: list = None) -> _base_model.BaseModel:
    """
    ...
    :param lr_reduce_points: optional points for learning rate reduction
    ...
    if lr_reduce_points is not None:
        model.lr_reduce_points = lr_reduce_points
...
def retrain_model_with_results_file(results_file_path: pathlib.Path, model_name: str, datasplit: str,
    ...
    :param saved_outerfold_number: outerfold results file if model is retrained with fixed hyperparameters of outerfold
    ...
    if dataset.featureset == 'features':
    ...
    if issubclass(helper_functions.get_mapping_name_to_class()[model_name], _torch_model.TorchModel):
        # additional attributes for torch and tensorflow models
        additional_attributes_dict['n_features'] = dataset.X_full.shape[1]
        additional_attributes_dict['size_alphabet'] = dataset.X_full.shape[-1]
    ...
    model.retrain(X_retrain=X_retrain, y_retrain=y_retrain)
    y_pred_test, y_score_test = model.predict(X_in=X_test)
    eval_scores = \
